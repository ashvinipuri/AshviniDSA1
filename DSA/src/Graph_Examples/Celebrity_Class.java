...
public class Celebrity_Class 
{
    ...
    int find_celeb()
    {
    	// read each column and find v-1 count then check row to zeroes
    	...
    	for( int c=0;c<v;c++)
    	{
    		...
    		boolean flag=true;
    		if(count==v-1) //all are pointing ti ith node
    		{
    			// cherck all columns and row
    			for(int row=0;row<v;row++)
    			{
    			
					int candidate = 0;
					if(g[candidate][row]!=0)
    				{
    					flag=false;
    					break;
    				}
